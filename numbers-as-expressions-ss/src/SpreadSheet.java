...
public class SpreadSheet extends JFrame {
	...
	public String evaluateToken(String tok, int depth) throws NumberFormatException {
	...
	protected String calculateUnary(StringTokenizer tokens, int depth) throws NumberFormatException {
		...
		while (tokens.hasMoreTokens()) {
			...
			if (tok.equals("-")) {
				...
				if (tok == null) return null;
				tok = isNegative ? multiply(tok, "-1") : tok;
				return tok;
			}
		}
		return null;
	...
	public String parseFormula(StringTokenizer tokens, int depth) throws NumberFormatException {
		if (tokens.hasMoreTokens()) {
			String tok = calculateUnary(tokens, depth);
			System.out.println(tok);
			if (tok == null) return null;
			while (tokens.hasMoreTokens()) {
				String tok2 = tokens.nextToken(); // Expects an operator
				if (tok2 == null || !tok2.matches("[\\+\\-\\*\\/]")) return null;
				if (!tokens.hasMoreTokens()) return null;
				String tok3 = calculateUnary(tokens, depth);
