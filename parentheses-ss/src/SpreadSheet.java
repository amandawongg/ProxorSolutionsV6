				...
				if (depth <= maxRows * maxCols) {
					...
					if (tokens.hasMoreTokens() && (tokens.nextToken().equals("="))) {
						...
						for (int i = 0; i < formula.length(); i++) {
							if (formula.charAt(i) == '(') openBracketCount++;
						}
						int closeBracketCount = 0;
						for (int i = 0; i < formula.length(); i++) {
							if (formula.charAt(i) == ')') closeBracketCount++;
						}
						
						String val;
						if (openBracketCount == closeBracketCount && !formula.contains("|")) {
							...
							while (tokens.hasMoreTokens()) {
								...
								if (tok.equals(")")) {
									...
									while (s.size() >= 2) {
										String tok2 = s.pop();
										String tok1 = s.pop();
										formulaSegment = tok2 + "|" + formulaSegment;
										if (tok1.equals("(")) {
											StringTokenizer tokensSegment = new StringTokenizer(formulaSegment, "|", false);
											String segmentAns = parseFormula(tokensSegment, depth);
											s.push(segmentAns);
											break;
										} else {
											formulaSegment = tok1 + "|" + formulaSegment;
										}
									}
								} else {
									s.push(tok);
							...
							String remainingToks = "";
							while (!s.isEmpty()) {
								remainingToks = s.pop() + "|" + remainingToks;
							}
							StringTokenizer tokensFinal = new StringTokenizer(remainingToks, "|", false);
							val = parseFormula(tokensFinal, depth);
						} else {
							val = null;
						}
